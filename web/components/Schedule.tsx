...
import { CheckCircle2, Radio, ScrollText, RotateCcw, Music } from "lucide-react";
  ...
  return (
    <div className="flex flex-col">
      {programme.map((slot, idx) => {
        ...
        const isMusic = slot.type_script === "music";
          ...
          <div key={slot.id} className="flex gap-3">
            <div className="flex flex-col items-center shrink-0 w-5">
              <div className={`mt-4 w-3 h-3 rounded-full shrink-0 flex items-center justify-center
                ${isCurrent
                  ? isMusic
                    ? "bg-purple-500 ring-4 ring-purple-100 shadow-sm"
            ...
            <div
              className={`flex-1 flex flex-col gap-1.5 py-2.5 px-3 rounded-xl mb-1 transition-all duration-200 cursor-pointer
                ${isCurrent && !isMusic ? "bg-orange-50 border-2 border-orange-300 shadow-sm" : ""}
                ${isCurrent && isMusic  ? "bg-purple-50 border-2 border-purple-300 shadow-sm" : ""}
              ...
              {isMusic ? (
                <div className="flex items-center gap-3">
                  <div className="w-4 shrink-0">
                    {isCurrent && <Radio className="w-4 h-4 text-purple-500 animate-pulse" />}
                    {isPast && <CheckCircle2 className="w-4 h-4 text-gray-300" />}
                  </div>
                  <span className={`text-xs font-mono font-semibold tabular-nums w-11 shrink-0 ${isPast ? "text-gray-300" : "text-gray-500"}`}>
                    {slot.start_time}
                  </span>
                  <Music className={`w-3 h-3 shrink-0 ${isCurrent ? "text-purple-500" : isPast ? "text-gray-300" : "text-purple-400"}`} />
                  <span className={`text-xs font-medium italic ${isCurrent ? "text-purple-700" : isPast ? "text-gray-400" : "text-purple-500"}`}>
                    Music break
                  </span>
                  {isCurrent && (
                    <AlertBadge color="red" label="On Air" pulse className="shrink-0 hidden sm:inline-flex ml-auto" />
                  )}
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-xs font-mono font-semibold tabular-nums w-11 shrink-0 ${isPast ? "text-gray-400" : "text-gray-500"}`}>
                      {slot.start_time}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs truncate mt-0.5 hidden sm:block ${isPast ? "text-gray-300" : "text-gray-400"}`}>
                        {slot.noms.join(" · ")}
                      </p>
                    </div>
                  ...
                  {(isCurrent || isPast) && (
                    <div className="flex items-center gap-2 pl-7">
                      <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden border border-gray-300">
                        <div
                          className={`h-full rounded-full transition-all duration-1000 ${isPast ? "bg-gray-300" : t.bar}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
